...
<body{if $body_id} id="{$body_id|escape}" {/if}{if $body_class} class="{$body_class|escape}" {/if}>
    ...
    <div class="main row">
        <div class="innermain container">
            <div class="s-row">
                {if 0
                < $boxes_left_side|@count} <div class="leftcol s-grid-3">
                        ...
                        <form action="{route key='productMailFriend' productId=$prod_id}" method="post" class="multirow">
                            <fieldset>
                                ...
                                <div class="shaded_inputwrap">
                                    <input type="text" name="from" id="from" value="{$data.from|escape}" size="30" class="{if $data_error.from}error{/if}"> {if $data_error.from}
                                    <ul class="error row">
                                ...
                                <div class="shaded_inputwrap">
                                    <input type="text" name="email" id="email" value="{$data.email|escape}" size="30" class="{if $data_error.email}error{/if}"> {if $data_error.email}
                                    <ul class="row error">
