...
<body{if $body_id} id="{$body_id|escape}" {/if}{if $body_class} class="{$body_class|escape}" {/if}>
    ...
    <div class="container content-wrapper clearfix">
        ...
        <div class="box" id="box_passchange">
            <h2 class="page-title">{translate key="Enter a new password"}</h2>


            <form action="{route key='passremindStep3'}" method="post" class="form-horizontal">
                <fieldset>
                    {include file='formantispam.tpl'}

                    <div class="row clearfix">
                        <div class="label">
                            <label for="pass1_input">{translate key="Password"}<em class="color">*</em>:</label>
                        </div>
                        <div class="input">
