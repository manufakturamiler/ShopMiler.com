<div class="submenu level{$level}">
    <ul class="level{$level}">
        ...
        <li class="{if count($cat->getActiveLangChildrenList())}parent{/if}" id="hcategory_{$cat->getIdentifier()|escape}">
            ...
            </h3> {if count($cat->getActiveLangChildrenList())} {assign var=uplevel value=$level+1} {include file='headermenu.tpl' level=$uplevel headermenu_categories=$cat->getActiveLangChildrenList()} {/if}
        </li>
        {/foreach}
    </ul>
</div>