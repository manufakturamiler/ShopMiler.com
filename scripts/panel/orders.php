                        ...
                        <table class="orders classic table">
                            <thead>
                                <tr>
                                    ...
                                    {if $config_confirm}
                                    <td class="status">{translate key='Confirmation'}</td>{/if}
                                    <td class="actions">{translate key='Actions'}</td>
                                </tr>
                            </thead>

                            <tbody>
                                {foreach from=$orders item=order name=list} {assign var=id value=$order->getIdentifier()}
                                <tr class="{if $smarty.foreach.list.index % 2}odd{else}even{/if}">
                                    ...
                                    <td class="actions">
                                        <a href="{route key='panelOrder' orderId=$id}" class="details btn btn-red spanhover" title="{translate key='szczegóły zamówienia'}">
                                            <img src="{baseDir}/public/images/1px.gif" alt="" class="px1">
                                            <span>{translate key='details'}</span>
                                        </a>

                                        {if $order->sumOrder() > $order->order->paid && $order->hasOnlinePayment()}
                                        <a href="{route key='panelPayment' orderId=$id}" class="payment btn spanhover">
                                            <img src="{baseDir}/public/images/1px.gif" alt="" class="px1">
                                            <span>{translate key='pay'}</span>
                                        </a>
                                        {/if} {if $order->invoice && true == $show_invoice}
