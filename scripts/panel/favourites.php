...
<body{if $body_id} id="{$body_id|escape}" {/if}{if $body_class} class="{$body_class|escape}" {/if}>
    ...
    <div class="main row">
        <div class="innermain container">
            <div class="s-row">
                {if 0
                < $boxes_left_side|@count} <div class="leftcol large s-grid-3">
            ...
            <div class="centercol {if ($boxes_left_side|@count == 0) and ($boxes_right_side|@count == 0)}s-grid-12{elseif 0 != $boxes_left_side|@count and $boxes_right_side|@count != 0}s-grid-6{else}s-grid-9{/if}">
                {dynamic} {foreach from=$boxes_top_side item=v key=k} {box file="../boxes/$v/box.tpl" box="$k"} {/foreach} {/dynamic} {if $articles && count($articles) > 0} {include file='news/listofarticles.tpl'} {/if}
                ...
                <div class="box" id="box_favourites">
                    ...
                    <div class="innerbox">
                        <table class="favourites classic table">
                            ...
                            <tbody>
                                {foreach from=$user->user->favourites item=product name=list} {assign var=id value=$product->getIdentifier()}
                                ...
                                <tr class="{if $smarty.foreach.list.index % 2}odd{else}even{/if}">
                                    ...
                                    <td class="cell-center price">
                                        {if $product->product->specialOffer} {currency value=$product->getSpecialOfferPrice()} {if $product->product->currency and $currency->getIdentifier() != $product->product->currency->getIdentifier()}
                                        <em class="default-currency">({currency id=$product->product->product->currency_id rate=1 value=$product->getCurrencySpecialOfferPrice()})</em> {/if} {else} {currency value=$product->getPrice()} {if $product->product->currency and $currency->getIdentifier() != $product->product->currency->getIdentifier()}
                                        <em class="default-currency">({currency id=$product->product->product->currency_id rate=1 value=$product->getCurrencyPrice()})</em> {/if} {/if}
                                    </td>

                                    {if 1 == $skin_settings->productdetails->availability}
                                    <td class="sum">
                                        {if $product->availability && $product->availability->translation} {if $product->availability->availability->photo}
                                        <img src="{baseDir}/{$product->availability->getUrl()}" alt="" /> {/if }{$product->availability->translation->name|escape} {else} - {/if}
                                    </td>
                                    {/if}

                                    <td class="actions">
                                        {if true == $enablebasket && 1 == (int) $product->availability->availability->can_buy}
                                        <a href="{route key=$basketAddRoute stockId=$id}" title="{translate key='add to cart'}" class="btn btn-red2 addtobasket spanhover">
                                            <img src="{baseDir}/public/images/1px.gif" alt="" class="px1" />
                                            <span>{translate key='add to cart'}</span>
                                        </a>
                                        {elseif $enable_availability_notifier && $product->product->isEnabledNotifier()} {dynamic} {assign var="availabilityNotifyUser" value=$product->getAvailabilityNotifyByUser()}
            ...
            {if 0
            < $boxes_right_side|@count} <div class="rightcol large s-grid-3">
