<div class="box box_custom" id="{if $boxNs->$box_id->css}{$boxNs->$box_id->css|escape}{else}box_custom{$box_id|escape}{/if}">
    {if 1 == $boxNs->$box_id->border} {if $boxNs->$box_id->title}
    <div class="boxhead">
        <span><img src="{baseDir}/public/images/1px.gif" alt="" class="px1">{$boxNs->$box_id->title|escape}</span>
    </div>
    {/if}
    <div class="innerbox">
        {/if} {if 0 == $boxNs->$box_id->mode} {$boxNs->$box_id->html} {/if} {if 1 == $boxNs->$box_id->mode} {$boxNs->$box_id->text} {/if} {if 2 == $boxNs->$box_id->mode} {if 'swf' == substr($boxNs->$box_id->img, -3)}
        <!--[if !IE]> -->
...
<![endif]-->
        {else}
        <img src="{baseDir}/{$boxNs->$box_id->img}" alt=""> {/if} {/if} {if 1 == $boxNs->$box_id->border}
    </div>
    {/if}
</div>