...
<div class="box box_banner" id="box_banner{$box_id}">
    ...
    <h5 class="boxintro">{$boxNs->$box_id->text}</h5>{/if} {foreach from=$boxNs->$box_id->banners item=banner} {if 'swf' == substr($banner->banner->file, -3)}
    <!--[if !IE]> -->
...
<![endif]-->
    {else}
    <a {if 1==( int) $banner->banner->blank}target="_blank"{/if} href="{route key='banner' bannerId=$banner->getIdentifier()}" title="{$banner->banner->name|escape}" class="banner">
